...
- 已在项目 `.dotnet` 准备隔离 .NET SDK `8.0.425`；v1.0.6 Release 构建通过，核心测试 4 项全部通过；self-contained 候选已生成。

## 状态

- 版本：`v1.0.6`（受控执行功能与候选发布提交版本，已写入 `VERSION.md`）
- 发布：未发布
- Git：已提交 `v1.0.5`（`2e2bf5f`）；本轮受控执行与候选发布待提交，持续启用提交，禁止自动 push
