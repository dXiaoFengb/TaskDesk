...
- 已确认四份文件包含一致的 TaskDesk 命名、资源库定位顺序、`data\\tasks.json`、根目录 `TaskDesk.exe`、主题规则和硬约束。
...
- 已在项目 `.dotnet` 准备隔离 .NET SDK `8.0.425`；v1.0.8 Release 构建通过，核心测试 4 项全部通过；根目录 self-contained 候选已生成。

## 状态

- 版本：`v1.0.8`（启动崩溃修复与根目录入口提交版本，已写入 `VERSION.md`）
- 发布：未发布
- Git：已提交 `v1.0.7`（`b76e039`）；本轮启动崩溃修复与根目录入口待提交，持续启用提交，禁止自动 push
...
4. 开展 Windows 11 UI/DPI 手工验证；补充真实环境下的普通权限启动验证和更多交互测试。
