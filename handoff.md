...
- 已在项目 `.dotnet` 准备隔离 .NET SDK `8.0.425`；窗口接入后的 Release 构建通过，核心测试 3 项全部通过。

## 状态

- 版本：`v1.0.4`（窗口接入提交版本，已写入 `VERSION.md`）
- 发布：未发布
- Git：已提交 `v1.0.3`（`2c14f68`）；本轮窗口接入待提交，持续启用提交，禁止自动 push
...
4. 运行 self-contained 发布并开展 Windows 11 手工 UI/DPI 验证；随后补充受控执行与更多交互测试。
