...
- 已在项目 `.dotnet` 准备隔离 .NET SDK `8.0.425`；v1.0.5 Release 构建通过，核心测试 3 项全部通过；self-contained 候选已生成。

## 状态

- 版本：`v1.0.5`（版本元数据与候选发布提交版本，已写入 `VERSION.md`）
- 发布：未发布
- Git：已提交 `v1.0.4`（`3f8eebc`）；本轮版本元数据与候选发布待提交，持续启用提交，禁止自动 push
...
4. 开展 Windows 11 UI/DPI 手工验证；补充资源库工具确认后普通权限启动与更多交互测试。
