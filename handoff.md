...
- 已在项目 `.dotnet` 准备隔离 .NET SDK `8.0.425`；主项目构建通过，核心测试 3 项全部通过。

## 状态

- 版本：`v1.0.3`（核心数据层提交版本，已写入 `VERSION.md`）
- 发布：未发布
- Git：已提交 `v1.0.2`（`34ab11f`）；本轮核心数据层待提交，持续启用提交，禁止自动 push
...
4. 将核心数据层接入窗口布局，继续实现资源库只读浏览/预览、主题读取和发布流程。
